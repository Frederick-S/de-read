...
module.exports.read = (function () {
    ...
    DefaultProcessor.prototype.oneDigit = function (n, options) {
        return (n === 1 ? (options.type === 'year' && this.singularYearPrefix ? this.singularYearPrefix : this.singularPrefix) : numbers[n]) + (n > 1 ? this.pluralSuffix : this.singularSuffix);
    };
    
    DefaultProcessor.prototype.twoDigits = function (n, options) {
        var s = '';
    ...
    DefaultProcessor.prototype.threeDigits = function (n, options) {
        if (n === 100) {
            ...
            if (hundert === 1) {
                hundertPart = (options.type === 'year' ? 'ein' : '') + 'hundert';
            } else {
                hundertPart = numbers[hundert] + 'hundert';
            }
    ...
    DefaultProcessor.prototype.process = function (node, options) {
        if (!node) {
            return '';
        }
        ...
        if (node.nodeType === this.type) {
            if (!this.readZero && node.number === 0) {
                return this.successor ? this.successor.process(node.next, options) : '';
            }
            ...
            switch (length) {
                case 1:
                    s = this.oneDigit(node.number, options);
                    break;
                case 2:
                    s = this.twoDigits(node.number, options);
                    break;
                case 3:
                    s = this.threeDigits(node.number, options);
                    break;
                default:
                    s = '';
                    break;
            ...
            return this.successor && !areLeftNodesZero(node.next) ? s + this.successor.process(node.next, options) : s;
        } else {
            return this.successor ? this.successor.process(node, options) : '';
        }
    };
    ...
    var read = function (n, options) {
        ...
        if (head) {
            ...
            return processor.process(head, options || {});
        } else {
            return '';
        }
    };
